...
interface RefundUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export function RefundUploadModal({ isOpen, onClose }: RefundUploadModalProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/refunds/upload`, {
        method: 'POST',
        body: formData,
      });
  ...
  const downloadTemplate = () => {
    const csvContent = 'transaction_id,refund_amount,refund_type\nTXN123,500000,refund\nTXN456,300000,chargeback';
    const blob = new Blob([csvContent], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'refund-template.csv';
    link.click();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Upload Refunds</h2>
            <p className="text-sm text-gray-500 mt-1">Upload CSV file with refund data</p>
          </div>
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <div className="p-3 bg-gray-50 border border-gray-200 rounded text-sm">
            ...
            <ul className="space-y-1 text-gray-600">
              ...
              <li><code className="bg-white px-1 py-0.5 rounded">refund_amount</code> - Amount in PAISE (₹1 = 100)</li>
              <li><code className="bg-white px-1 py-0.5 rounded">refund_type</code> - "refund" or "chargeback"</li>
            </ul>
          </div>
