...
// Proxy /api/recon requests to Recon API on port 5103
app.use('/api/recon', createProxyMiddleware({
  target: 'http://localhost:5103/recon',
  changeOrigin: true,
  pathRewrite: (path, req) => {
    const newPath = path.replace('/api/recon', '');
    console.log(`[Proxy] ${req.method} ${path} -> http://localhost:5103/recon${newPath}`);
    return newPath;
  },
  onError: (err, req, res) => {
    console.error('[Proxy Error]', err.message);
    res.status(502).json({ error: 'Recon API unavailable', details: err.message });
  }
}));
