// ============================================================================
// DATABASE-DRIVEN V1→V2 MAPPING (SINGLE SOURCE OF TRUTH)
// ============================================================================
 ...
 */
function buildV2MappingFromDBConfig(bankConfig) {
  ...
  if (v1Mappings.transaction_id) {
    ...
    v2Mapping[normalizedKey] = 'bank_ref';
  }
  ...
  console.log(`[V1 Mapper] Built V2 mapping for ${bankConfig.bank_name}:`, v2Mapping);
...
function mapV1ToV2(v1Row, type = 'pg_transactions', dbMapping = null) {
  ...
  if (dbMapping) {
    ...
    console.log('[V1 Mapper] Using hardcoded mapping (fallback)');
...
async function convertV1CSVToV2(csvData, type = 'pg_transactions', bankName = null) {
  ...
  console.log(`[V1 Mapper] Converting V1 format to V2 for type: ${type}, bank: ${bankName || 'N/A'}`)
  ...
  if (type === 'bank_statements' && bankName) {
    ...
    if (bankConfig) {
      dbMapping = buildV2MappingFromDBConfig(bankConfig);
    } else {
  ...
  return csvData.map((row, idx) => {
    try {
      const v2Row = mapV1ToV2(row, type, dbMapping);
