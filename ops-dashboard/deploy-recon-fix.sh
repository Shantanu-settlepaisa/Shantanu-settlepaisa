...
###############################################################################
# Deploy Reconciliation Fix - Run this script on EC2
# Copy-paste this entire script into AWS EC2 Instance Connect terminal
###############################################################################

set -e  # Exit on error

echo "🚀 Starting deployment of reconciliation fix..."
echo ""
...
echo "📝 Step 1: Creating backfill script..."
cat > backfill-reconciliation-statuses.cjs << 'BACKFILL_SCRIPT_EOF'
#!/usr/bin/env node
...
chmod +x backfill-reconciliation-statuses.cjs
echo "✅ Backfill script created"
echo ""

# Step 2: Backup current reconciliation job file
echo "💾 Step 2: Backing up current runReconciliation.js..."
cp services/recon-api/jobs/runReconciliation.js services/recon-api/jobs/runReconciliation.js.backup-oct26
echo "✅ Backup created: runReconciliation.js.backup-oct26"
echo ""
...
if [ $BACKFILL_STATUS -eq 0 ]; then
  ...
  echo "❌ Backfill failed with exit code $BACKFILL_STATUS"
...
echo ""
echo "📋 Next Steps:"
echo "  1. The fixed runReconciliation.js needs to be uploaded manually"
echo "  2. Then restart recon-api: pm2 restart recon-api"
echo "  3. Check dashboard: http://shantanu-settlepaisa-ops-staging.s3-website.ap-south-1.amazonaws.com/ops/overview"
echo ""
